...
ZeissUByte g_XYWheel = 0x25; //not included in source code
...
int ZeissAxis::SetTrajectoryVelocity(MM::Device& device, MM::Core& core, ZeissUByte devId, long velocity)
{
   ...
   //cout << "setting ZeissAxis velocity to be " << velocity << '\n'; //print statement
   memcpy(command+6, &tmp, ZeissLongSize); 
   
   ret = g_hub.ExecuteCommand(device, core,  command, commandLength);
   //cout << "result of executing velocity command is" << ret << '\n';
   if (ret != DEVICE_OK)
	  //cout << "for some reason I can't set velocity to be" << velocity;
      return ret;
   g_hub.SetModelBusy(devId, true);

   //cout << "DEVICE_OK in the velocity function is" << DEVICE_OK << '\n';
   return DEVICE_OK;
}
...
int ZeissAxis::SetTrajectoryAcceleration(MM::Device& device, MM::Core& core, ZeissUByte devId, long acceleration)
{
   ...
   //cout << "setting ZeissAxis acceleration to be " << acceleration << '\n'; //print statement
   memcpy(command+6, &tmp, ZeissLongSize); 
   ret = g_hub.ExecuteCommand(device, core,  command, commandLength);
   //cout << "result of executing acceleration command is" << ret << '\n';
   if (ret != DEVICE_OK){
	  //cout << "for some reason I can't set acceleration to be" << acceleration;
      return ret;
   }
   g_hub.SetModelBusy(devId, true);
   //cout << "DEVICE_OK in the acceleration function is" << DEVICE_OK << '\n';
   return DEVICE_OK;
}
...
int Axis::Initialize()
{
   ...
   bool hasTV = false;
   ret = HasTrajectoryVelocity(*this, *GetCoreCallback(), g_FocusAxis, hasTV);
   if (ret != DEVICE_OK)
      return ret;
   if (hasTV) 
   {
      pAct = new CPropertyAction(this, &Axis::OnTrajectoryVelocity);
      ret = CreateProperty("Velocity (micron/s)", "0", MM::Float, false, pAct);
      if (ret != DEVICE_OK)
         return ret;
      pAct = new CPropertyAction(this, &Axis::OnTrajectoryAcceleration);
...
int Axis::OnTrajectoryVelocity(MM::PropertyBase* pProp, MM::ActionType eAct)
{
   if (eAct == MM::BeforeGet) {
      long velocity;
      int ret = ZeissAxis::GetTrajectoryVelocity(*this, *GetCoreCallback(), g_FocusAxis, (ZeissLong&) velocity);
	  cout << "Z Trajectory velocity for MM BeforeGet is " << velocity << '\n';
      if (ret != DEVICE_OK)
         return ret;
	  ...
	  cout << "Z Trajectory velocity for MM AfterSet is " << velocity << '\n';
      int ret = ZeissAxis::SetTrajectoryVelocity(*this, *GetCoreCallback(), g_FocusAxis, (ZeissLong) velocity);
      if (ret != DEVICE_OK)
         return ret;
...
int Axis::OnTrajectoryAcceleration(MM::PropertyBase* pProp, MM::ActionType eAct)
{
   if (eAct == MM::BeforeGet) {
      // we are lazy and only check the x axis
      long accel;
      int ret = ZeissAxis::GetTrajectoryAcceleration(*this, *GetCoreCallback(), g_FocusAxis, (ZeissLong&) accel);
      if (ret != DEVICE_OK)
         return ret;
      pProp->Set( (float) (accel / 1000.0) );
	  cout << "Z trajectory acceleration for MM axis BeforeGet is " << accel << '\n' ;
   } else if (eAct == MM::AfterSet) {
      double tmp;
      pProp->Get(tmp);
      long accel = (long) (tmp * 1000.0);
	  cout << "Z trajectory acceleration for MM Axis AfterSet is " << accel << '\n' ;
      int ret = ZeissAxis::SetTrajectoryAcceleration(*this, *GetCoreCallback(), g_FocusAxis, (ZeissLong) accel);
      if (ret != DEVICE_OK)
         return ret;
   }
...
int XYStage::OnVelocity(MM::PropertyBase* pProp, MM::ActionType eAct)
{
   if (eAct == MM::BeforeGet)   {
	  ...
	  //cout << "velocity for MM BeforeGet is" << velocity_  << '\n' ;
   }
   else if (eAct == MM::AfterSet)                             
   {  
      string result;                                             
      ...
      if (result == default_)
         ...
         velocity_ = 8;
	 // cout << "velocity for MM After Set is" << velocity_ << '\n' ;
   }                                                                                                                     
   return DEVICE_OK;                                          
}
...
int XYStage::OnTrajectoryVelocity(MM::PropertyBase* pProp, MM::ActionType eAct)
{
   if (eAct == MM::BeforeGet) {
	  ...
	  cout << "X Trajectory velocity for MM BeforeGet is " << velocity << '\n';
      if (ret != DEVICE_OK)
         return ret;
	  ret = ZeissAxis::GetTrajectoryVelocity(*this, *GetCoreCallback(), g_StageYAxis, (ZeissLong&) velocity); //added check for y axis
	  cout << "Y Trajectory velocity for MM BeforeGet is " << velocity << '\n';
      if (ret != DEVICE_OK)
         return ret;
      pProp->Set( (float) (velocity/1000.0) );
   } else if (eAct == MM::AfterSet) {
      ...
      pProp->Get(tmp);
      long velocity = (long) (tmp * 1000.0);
	  cout << "XY Trajectory velocity for MM AfterSet is " << velocity << '\n';
      int ret = ZeissAxis::SetTrajectoryVelocity(*this, *GetCoreCallback(), g_StageXAxis, (ZeissLong) velocity);
	  //cout << "ret for the X axis in the XYStage::OnTrajectoryVelocity function is" << ret << '\n';
      if (ret != DEVICE_OK)
         return ret;
...
int XYStage::OnTrajectoryAcceleration(MM::PropertyBase* pProp, MM::ActionType eAct)
{
   if (eAct == MM::BeforeGet) {
      ...
      int ret = ZeissAxis::GetTrajectoryAcceleration(*this, *GetCoreCallback(), g_StageXAxis, (ZeissLong&) accel);
	   cout << "X Trajectory acceleration for MM BeforeGet is " << accel << '\n';
      if (ret != DEVICE_OK)
         return ret;
	  ret = ZeissAxis::GetTrajectoryAcceleration(*this, *GetCoreCallback(), g_StageYAxis, (ZeissLong&) accel); //added check for y axis
	  cout << "Y Trajectory acceleration for MM BeforeGet is " << accel << '\n';
      if (ret != DEVICE_OK)
         return ret;
      pProp->Set( (float) (accel / 1000.0) );
   } else if (eAct == MM::AfterSet) {
      ...
      pProp->Get(tmp);
      long accel = (long) (tmp * 1000.0);
	  cout << "XY trajectory acceleration for MM AfterSet is " << accel << '\n' ;
