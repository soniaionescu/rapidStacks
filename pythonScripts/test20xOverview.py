...
from testImageProcessing import flatFieldCorrection, moveForFlatField
...
"""
def getOverviewAndSaveMIP(folder, RightX, LeftX, BottomY, TopY, upperZ, lowerZ, flatField, mmc):
    ...
    rowWidth = (numberImagesArrayRow[len(numberImagesArrayRow)-1] + 1)*int(imageWidth*notOverlap)  ## need to add 1 because we need the total size of the array, if we don't add 1 it's the size of the array up to the last picture
        ...
        if(iteration%2==0): ## it is an even iteration
            ...
            for x in range(0, len(imageWidthArray)): 
                ...
                if configuration.saveImages == True:
                    ...
                    array, hvLayer = makeMIP(upperZ, lowerZ, flatField, mmc, folderOfSavedImages)
                else:
                ...
                croppedArray = array[0:int(imageHeight*notOverlap):1, 0:int(imageWidth*notOverlap):1]
...
if __name__ == '__main__':
    ...
    getOverviewAndStitch("813TestOverview", RightX, LeftX, BottomY, TopY, flatField, mmc)
